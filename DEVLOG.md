...
## 2026-09-02 — Export-byte rebaseline: the manifest was stale, not the export

A normal deploy — `npm run deploy:web`, **no `SKIP_EXPORT`** — ran a full Firestore export. The
export succeeded, and `deploy-web.sh` then refused to publish because it had changed two tracked
files. **Nothing was published.** That refusal is the gate working exactly as designed.

Owner ruling: accept the export's bytes as canonical. The evidence, re-proved before installing
them, is written up in full at `audit/export-byte-rebaseline-2026-09-02.md`. In short:

**`posts.json`** — 4,966 documents both sides, none added or removed, row order identical
position-by-position, **10,400,391 bytes both sides**, **zero value differences**. Exactly one row
differs, by property order alone: `posts[#4686].postAnalysis`, where `contextUnits` and
`predictionSpans` swap places. #4686 is the drop that gained a Prediction under "WATERGATE x1000",
so it is the only row where both keys were appended in one run and their relative order can differ
between the rebuild path and the export path. The certification manifest's `semanticSha256` for
`posts.json` is unchanged across the rebaseline, which is an independent confirmation.

**`manifest.json`** — `totalBytes` 12,340,347 → 12,340,575, **+228**, nothing else. It reconciles
exactly, from git:

```
posts.json  7f73af5 (pre-#4686)  10,400,163      manifest.totalBytes  7f73af5  12,340,347
posts.json  224ef91 (#4686)      10,400,391      manifest.totalBytes  224ef91  12,340,347  ← stale
                          +228                   regenerated by export         12,340,575  +228
```

Commit `224ef91` grew `posts.json` by 228 bytes and did not recompute the aggregate. It stayed
wrong through `9136952` — the live build — because the 2026-09-02 deploy shipped under
`SKIP_EXPORT=1`. The first ordinary export since corrected it.

**Determinism proved before committing.** The preserved export bytes were installed and
`rebuild-bundle.mjs` run twice: both exit 0, identical output, and both files hold their export
hashes. The chain reproduces the export's serialisation rather than fighting it.

**Refreshed, never hand-edited.** `audit/certification-manifest.json` regenerated through its own
generator — exactly two fields moved, `certifiedAt` and `artifacts["posts.json"].sha256`. Counts,
`canonical`, `seedVersion` and every `semanticSha256` unchanged; the generator refuses to write at
all if a canonical count has drifted. Cross-section re-run with all three private editorial queues
present: **222/222, 0 skipped**. `SEED_VERSION` 117. #4686 still a Prediction. Pictures still
1,690 — 1,653 complete, 29 partial, 8 withheld.

**The gap this exposed.** `totalBytes` was wrong across two deploys and no gate noticed, including
