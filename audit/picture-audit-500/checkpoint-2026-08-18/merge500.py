# Merge agent JSONL outputs into public/data/picture-analysis.json.
# Usage:
#   python merge500.py check     - validate + report gaps, write nothing
#   python merge500.py compile   - merge validated entries into the app data file
...
batch = {r['n']: r for r in json.load(open(os.path.join(SCRATCH, 'batch500.json'), encoding='utf-8'))}
entries, problems, dupes = {}, [], []
for path in sorted(glob.glob(os.path.join(SCRATCH, 'agent-out', '*.jsonl'))):
