# Fetch the batch1690 rows whose image is not in media-bundle, into fetched/.
...
# Updates batch1690.json file fields to "FETCHED:<basename>" on success.
...
batch = json.load(open(os.path.join(SCRATCH, 'batch1690.json'), encoding='utf-8'))
ok, fail = 0, []
for r in batch:
    if r['file'] != 'FETCH':
        continue
...
json.dump(batch, open(os.path.join(SCRATCH, 'batch1690.json'), 'w', encoding='utf-8'), indent=1)
