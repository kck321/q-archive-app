...
# Updates batch1450.json file fields to "FETCHED:<basename>" on success.
...
batch = json.load(open(os.path.join(SCRATCH, 'batch1450.json'), encoding='utf-8'))
ok, fail = 0, []
for r in batch:
    if r['file'] != 'FETCH':
        continue
...
json.dump(batch, open(os.path.join(SCRATCH, 'batch1450.json'), 'w', encoding='utf-8'), indent=1)
