...
# Mirrors merge850.py exactly, but reads batch1450.json and agent-out-1450/*.jsonl.
# Usage:
...
batch = {r['n']: r for r in json.load(open(os.path.join(SCRATCH, 'batch1450.json'), encoding='utf-8'))}
entries, problems, dupes = {}, [], []
for path in sorted(glob.glob(os.path.join(SCRATCH, 'agent-out-1450', '*.jsonl'))):
