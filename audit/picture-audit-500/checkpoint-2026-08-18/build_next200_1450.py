...
# next 200 rows (n=1251-1450) as batch1450.json — but ONLY after proving the reproduction
# matches first100.json (n=1-100), batch500.json (n=101-600), batch850.json (n=601-850)
# batch1050.json (n=851-1050) AND batch1250.json (n=1051-1250) hash-for-hash.
import json, os, sys
...
# ---- verification against the existing 1050 rows ----
...
existing = {r['n']: r for r in first100 + batch500 + batch850 + batch1050 + batch1250}
mismatch = 0
...
print(f'verification: {1250 - mismatch}/1250 rows reproduced exactly; total distinct images {len(order)}')
if mismatch:
    sys.exit('REFUSING to emit batch1450.json — enumeration does not reproduce the existing batches')

# ---- resolve local files for n=1251-1450 via the media-bundle manifest ----
...
for n in range(1251, 1451):
    ...
    if r['video']:
        videos.append(n)
    out.append(r)
json.dump(out, open(os.path.join(SCRATCH, 'batch1450.json'), 'w', encoding='utf-8'), indent=1)
print(f'batch1450.json: {len(out)} rows (n {out[0]["n"]}-{out[-1]["n"]})')
