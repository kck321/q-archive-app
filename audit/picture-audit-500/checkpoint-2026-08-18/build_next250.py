...
# next 250 rows (n=601-850) as batch850.json — but ONLY after proving the reproduction
# matches first100.json (n=1-100) and batch500.json (n=101-600) hash-for-hash.
import json, os, sys
...
for i, h in enumerate(order, 1):
    byhash[h]['n'] = i
    # occurrences accumulate in lexicographic scan order; the established format lists them
    # numerically (attached before referenced within a post, preserved by stable sort)
    byhash[h]['posts'].sort(key=lambda x: x['num'])

# ---- verification against the existing 600 rows ----
...
existing = {r['n']: r for r in first100 + batch500}
mismatch = 0
for n in range(1, 601):
    ...
    if got['hash'] != want['hash']:
        ...
        # posts list must match too (occurrence identity)
...
print(f'verification: {600 - mismatch}/600 rows reproduced exactly; total distinct images {len(order)}')
if mismatch:
    sys.exit('REFUSING to emit batch850.json — enumeration does not reproduce the existing batch')

# ---- resolve local files for n=601-850 via the media-bundle manifest ----
...
for n in range(601, 851):
    ...
    if r['video']:
        videos.append(n)
    out.append(r)
json.dump(out, open(os.path.join(SCRATCH, 'batch850.json'), 'w', encoding='utf-8'), indent=1)
print(f'batch850.json: {len(out)} rows (n {out[0]["n"]}-{out[-1]["n"]})')
