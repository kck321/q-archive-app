...
# Mirrors merge850.py exactly, but reads batch1250.json and agent-out-1250/*.jsonl.
# Usage:
...
batch = {r['n']: r for r in json.load(open(os.path.join(SCRATCH, 'batch1250.json'), encoding='utf-8'))}
entries, problems, dupes = {}, [], []
for path in sorted(glob.glob(os.path.join(SCRATCH, 'agent-out-1250', '*.jsonl'))):
