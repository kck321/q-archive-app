...
# next 200 rows (n=1051-1250) as batch1250.json — but ONLY after proving the reproduction
# matches first100.json (n=1-100), batch500.json (n=101-600), batch850.json (n=601-850)
# AND batch1050.json (n=851-1050) hash-for-hash.
import json, os, sys
...
existing = {r['n']: r for r in first100 + batch500 + batch850 + batch1050}
mismatch = 0
...
print(f'verification: {1050 - mismatch}/1050 rows reproduced exactly; total distinct images {len(order)}')
if mismatch:
    sys.exit('REFUSING to emit batch1250.json — enumeration does not reproduce the existing batches')

# ---- resolve local files for n=1051-1250 via the media-bundle manifest ----
...
for n in range(1051, 1251):
    ...
    if r['video']:
        videos.append(n)
    out.append(r)
json.dump(out, open(os.path.join(SCRATCH, 'batch1250.json'), 'w', encoding='utf-8'), indent=1)
print(f'batch1250.json: {len(out)} rows (n {out[0]["n"]}-{out[-1]["n"]})')
