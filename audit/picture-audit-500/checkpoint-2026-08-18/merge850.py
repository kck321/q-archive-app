...
# Mirrors merge500.py exactly, but reads batch850.json and agent-out-850/*.jsonl.
# Usage:
...
batch = {r['n']: r for r in json.load(open(os.path.join(SCRATCH, 'batch850.json'), encoding='utf-8'))}
entries, problems, dupes = {}, [], []
for path in sorted(glob.glob(os.path.join(SCRATCH, 'agent-out-850', '*.jsonl'))):
