...
| ✅ | Directives = 2,552 | 3304 |
| ✅ | Claims = 4,189 | 10258 |
| ✅ | Predictions = 630 | 934 |
| ✅ | Evidence = 6,590 | 6590 |
| ✅ | Entities = 1,335 canonical | 1532 |
| ✅ | Entities = 7,903 resolved mentions (headline) | 9364 |
| ✅ | core registry submetric = 5,273 | 5297 |
| ✅ | adjudicated tail submetric = 3,476 | 3015 |
...
| ✅ | submetrics sum to the headline | 5297 + 3015 + 1052 = 9364 |
| ✅ | the artifact ships the headline figure | 9364 |
...
| ✅ | canonical and mentions are distinct metrics | 1532 vs 9364 |
...
| ✅ | directives: quoted-block over-extension stays at its known 115 occurrences | 115 of 3304 (expected 115) |
| ✅ | claims: quoted-block over-extension stays at its known 721 occurrences | 721 of 10258 (expected 721) |
...
| ✅ | sectionInfo states 3,304 | ok |
| ✅ | sectionInfo states 10,258 | ok |
| ✅ | sectionInfo states 6,590 | ok |
| ✅ | sectionInfo headlines 1,532 entities and 9,364 mentions | ok |
...
| ✅ | Entities headline = certified mentions / 2,117 posts | ok |
...
| ✅ | none of the held mentions is counted in Entities | 9364 |
...
| ✅ | entity totals are unchanged by the import | 1532 entities / 9364 mentions |
...
| ✅ | the provenance audit covers every certified occurrence | 10459 of 10459 |
| ✅ | the applied totals are the proven ones plus every ruling recorded since | 1532/9364 against 1532/9364 |
...
| ✅ | every ambiguous occurrence is retained unchanged | 86 ambiguous, 0 acted on |
...
| ✅ | image-unconfirmed occurrences keep their certified mention | 40 held |
| ✅ | every one of them is in the private provenance queue | 40 queued |
...
| ✅ | the provenance audit describes the state it was run against | 1751/10459 against 1751/10459 |
...
| ✅ | the approved audit still holds its exact bytes | audit unchanged; inputs moved under 8 recorded decision(s) |
...
| ✅ | public canonical identities = 1,201 | view 1532 / registry 1532 |
| ✅ | certified prose mentions = 9,364 | view 9364 / registry sum 9364 |
| ✅ | displayed breakdown adds exactly to the canonical total | 1398 named in Q’s prose + 134 linked as a source only = 1532 |
| ✅ | prose and source-only components share no identity | 1398 + 134, overlap 0 |
...
| ✅ | the 229 dormant identities are reserved and never public | 229 reserved, 0 leaked |
| ✅ | no alias is published as its own row beside its canonical | none |
| ✅ | no occurrence is claimed by two identities | 10381 occurrences, 0 double-claimed |
| ✅ | per-post mention counts stay inside the identity that earned them | clean |
| ✅ | row count and identity count reconcile through the merge model | 1518 rows / 1532 identities / 25 in 11 merged rows |
...
| | Invariant | Observed |
|---|---|---|
| ✅ | postAnalysis entries equal certified mentions | 9364 vs 9364 |
...
| ✅ | analysis map totals reconcile with certified Directives | 3304 |
...
| ✅ | in-post repeats are indexed as separate occurrences | 10258 rows, 10151 distinct (post, text) |
...
| Q Directives | 3,304 | every actionRequests string across all posts | None. Every actionRequests entry is a certified directive. | n/a |
...
| Q Entities | 9,364 | sum of per-entity mention counts | Canonical entities (1,332) and mentions (7,903) are DIFFERENT metrics, not a row-count mismatch — one entity is mentioned many times. The headline covers every resolved mention: 4,463 from the 93-entity core registry plus 3,440 from the 1,239 adjudicated-tail entities. Unresolved alias tokens are counted in neither. | An unresolved alias must never be shown as a resolved identification. |
