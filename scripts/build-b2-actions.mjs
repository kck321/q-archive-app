...
//
// Writes audit/step3b1-b2-actions.jsonl. apply-step3b1.mjs executes these through the SAME code
...
for (const r of analysis.rows) {
  ...
  if (r.shape === 'TRIM_TO_ONE_SENTENCE') {
    ...
    actions.push({
      ...common,
      actionId: `B2-TRIM-${r.postNum}-${r.kind}-${r.start}-${r.end}`,
      kind: 'SPAN_TRIM',
  ...
  if (r.shape === 'ENTIRELY_NON_SEMANTIC' || r.shape === 'NO_SENTENCE_TOUCHED') {
    actions.push({
      ...common,
      actionId: `B2-DROP-${r.postNum}-${r.kind}-${r.start}-${r.end}`,
...
fs.writeFileSync(path.join(OUT, 'step3b1-b2-actions.jsonl'), actions.map(a => JSON.stringify(a)).join('\n') + '\n')

const tally = {}
for (const a of actions) tally[`${a.kind} / ${a.ruleCode}`] = (tally[`${a.kind} / ${a.ruleCode}`] ?? 0) + 1
console.log(`-> audit/step3b1-b2-actions.jsonl   ${actions.length} actions`)
