...
console.log(`   overlap marks: ${JSON.stringify(after.overlapMarks)}`)
...
const checks = [
  ...
  ["'real' kept as a Claim+Emphasis overlap", (after.overlapMarks ?? []).some(m => m.t === "'real'" && /claim/.test(m.title) && /emphasis/.test(m.title))],
]
console.log('\n  RESULT')
let failed = 0
